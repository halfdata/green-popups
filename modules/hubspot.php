...
class lepopup_hubspot_class {
		...
		if (current_user_can('manage_options')) {
			...
			<div class="lepopup-properties-item">
				<div class="lepopup-properties-label">
					<label>'.esc_html__('Private App Access Token', 'lepopup').'</label>
				</div>
				<div class="lepopup-properties-tooltip">
					<i class="fas fa-question-circle lepopup-tooltip-anchor"></i>
					<div class="lepopup-tooltip-content">'.esc_html__('Enter your HubSpot Private App Access Token. Go to "Integrations >> Private Apps" in your HubSpot account and click "Create a private app". Set the following scopes for app: crm.lists.read, crm.lists.write, crm.objects.contacts.read, crm.objects.contacts.write.', 'lepopup').'</div>
				</div>
	...
	function admin_lists() {
		...
		if (current_user_can('manage_options')) {
			...
			$offset = 0;
			while ($has_more) {
				$result = $this->connect($deps['api-key'], 'contacts/v1/lists?count=250&offset='.$offset);
				...
				if (array_key_exists('has-more', $result) && $result['has-more']) {
					$offset += 250;
				} else {
					$has_more = false;
				}
	...
	function get_fields_html($_key, $_fields) {
		global $wpdb, $lepopup;
		$result = $this->connect($_key, 'crm/v3/properties/contacts/');
		$fields_html = '';
		if (!empty($result) && is_array($result) && array_key_exists('results', $result)) {
			...
			<table>';
					foreach ($result['results'] as $field) {
						if (is_array($field)) {
						if (array_key_exists('name', $field) && array_key_exists('label', $field) && array_key_exists('hidden', $field)) {// && !$field['hidden'] && array_key_exists('formField', $field) && $field['formField']) {
								$fields_html .= '
				<tr>
	...
	function connect($_api_key, $_path, $_data = array(), $_method = '') {
		...
		try {
			$url = 'https://api.hubapi.com/'.ltrim($_path, '/');
			$curl = curl_init($url);
