...
class lepopup_sendgrid_class {
	...
	function admin_settings_html() {
		...
		if (current_user_can('manage_options')) {
			...
			if (!empty($data['api-key'])) {
				$fields_data = $this->get_fields_html($data['api-key'], $data['fields'], $data['custom_fields']);
		...
		if (current_user_can('manage_options')) {
			...
			if (is_array($result) && !empty($result)) {
				if (array_key_exists('errors', $result)) {
					...
					$lists['0'] = esc_html__('Global', 'lepopup');
	...
	function admin_fields_html() {
		...
		if (current_user_can('manage_options')) {
			...
			$return_object = $this->get_fields_html($deps['api-key'], $this->default_parameters['fields'], $this->default_parameters['custom_fields']);
			echo json_encode($return_object);
		}
		exit;
	}

	function get_fields_html($_key, $_fields, $_custom_fields) {
		global $wpdb, $lepopup;
			...
			<table>';
				...
				<tr>
					...
					foreach ($result['custom_fields'] as $field) {
						if (is_array($field)) {
							if (array_key_exists('name', $field) && !array_key_exists('read_only', $field)) {
								$fields_html .= '
				<tr>
					<th>'.esc_html($field['name']).'</th>
					<td>
						<div class="lepopup-input-shortcode-selector">
							<input type="text" name="custom_fields['.esc_html($field['name']).']" value="'.esc_html(array_key_exists($field['name'], $_custom_fields) ? $_custom_fields[$field['name']] : '').'" class="widefat" />
							<div class="lepopup-shortcode-selector" onmouseover="lepopup_shortcode_selector_set(this)";><span><i class="fas fa-code"></i></span></div>
						</div>
						<label class="lepopup-integrations-description">'.esc_html($field['name']).'</label>
					</td>
	...
	function front_submit($_result, $_data) {
		...
		$post_data = array(
			'list_ids' => $data['list-id'] != 0 ? array($data['list-id']) : array(),
			'contacts' => array(
				array(
		...
		if (sizeof($data['custom_fields']) > 0) {
			$post_data['contacts'][0]['custom_fields'] = array();
			foreach ($data['custom_fields'] as $key => $value) {
				$post_data['contacts'][0]['custom_fields'][$key] = $value;
			}
		}
