...
class lepopup_ajax_class {
	function __construct() {
		if (is_admin()) {
			...
			add_action('wp_ajax_lepopup-integrations-log-delete', array(&$this, "admin_integrations_log_delete"));
			add_action('wp_ajax_lepopup-bulk-integrations-log-delete', array(&$this, "admin_bulk_integrations_log_delete"));
			add_action('wp_ajax_lepopup-integrations-log-details', array(&$this, "admin_integrations_log_details"));
	...
	function admin_integrations_log_delete() {
		global $wpdb, $lepopup;
		if (current_user_can('manage_options') || $lepopup->demo_mode) {
			$callback = '';
			if (isset($_REQUEST['callback'])) {
			...
			if (array_key_exists('record-id', $_REQUEST)) {
				$record_id = intval($_REQUEST['record-id']);
				$record_details = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."lepopup_integrations_log WHERE deleted = '0' AND id = '".esc_sql($record_id)."'", ARRAY_A);
				if (empty($record_details)) $record_id = null;
			}
			if (empty($record_id)) {
				$return_data = array(
			...
			$wpdb->query("UPDATE ".$wpdb->prefix."lepopup_integrations_log SET deleted = '1' WHERE deleted = '0' AND id = '".esc_sql($record_details['id'])."'");
	...
	function admin_bulk_integrations_log_delete() {
		global $wpdb, $lepopup;
		if (current_user_can('manage_options') || $lepopup->demo_mode) {
			$callback = '';
			if (isset($_REQUEST['callback'])) {
			...
			$wpdb->query("UPDATE ".$wpdb->prefix."lepopup_integrations_log SET deleted = '1' WHERE deleted = '0' AND id IN ('".implode("','", $records)."')");
	...
	function admin_integrations_log_details() {
		global $wpdb, $lepopup;
		if (current_user_can('manage_options') || $lepopup->demo_mode) {
			$callback = '';
			if (isset($_REQUEST['callback'])) {
			...
			<table class="lepopup-record-details-table">
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Provider', 'lepopup').'</td><td class="lepopup-record-details-table-value">'.esc_html($record_details['provider']).'</td></tr>
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Endpoint', 'lepopup').'</td><td class="lepopup-record-details-table-value">'.esc_html($record_details['url']).'</td></tr>
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Method', 'lepopup').'</td><td class="lepopup-record-details-table-value">'.esc_html($record_details['method']).'</td></tr>
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Request Headers', 'lepopup').'</td><td class="lepopup-record-details-table-value">';
			$request_headers = json_decode($record_details['request_headers'], true);
			if (!empty($request_headers)) {
				$headers = array();
				foreach ($request_headers as $header) {
					$headers[] = esc_html($header);
				}
				$html .= '<textarea readonly="readonly" onclick="this.focus();this.select();">'.implode("\n", $headers).'</textarea>';
			} else {
				$html .= '-';
			}
			$html .= '
				</td></tr>
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Request Content Type', 'lepopup').'</td><td class="lepopup-record-details-table-value">'.esc_html($record_details['request_content_type']).'</td></tr>
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Request Content', 'lepopup').'</td><td class="lepopup-record-details-table-value"><textarea readonly="readonly" onclick="this.focus();this.select();">'.esc_html($record_details['request_content']).'</textarea></td></tr>
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Response HTTP Code', 'lepopup').'</td><td class="lepopup-record-details-table-value">'.esc_html($record_details['response_status_code']).'</td></tr>
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Response Content Type', 'lepopup').'</td><td class="lepopup-record-details-table-value">'.esc_html($record_details['response_content_type']).'</td></tr>
				<tr><td class="lepopup-record-details-table-name">'.esc_html__('Response Content', 'lepopup').'</td><td class="lepopup-record-details-table-value"><textarea readonly="readonly" onclick="this.focus();this.select();">'.esc_html($record_details['response_content']).'</textarea></td></tr>
			</table>
			</div>';
					...
					if ($errors_all === false) {
						...
						if (empty($errors)) {
							...
							if ($next_page_id === false) {
								...
								$hash = '';
								$record_details = null;
								if ($form_object->form_options['antibot-enable'] == 'on' && ($form_object->form_options['antibot-check-form'] == 'on' || $form_object->form_options['antibot-check-ip'] == 'on')) {
									$hash = md5(($form_object->form_options['antibot-check-form'] == 'on' ? json_encode($form_object->form_data) : '').($form_object->form_options['antibot-check-ip'] == 'on' ? $_SERVER['REMOTE_ADDR'] : ''));
									$record_details = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."lepopup_records WHERE deleted = '0' AND form_id = '".esc_sql($form_object->id)."' AND hash = '".esc_sql($hash)."' AND created > '".(time()-intval($form_object->form_options['antibot-delay']))."'", ARRAY_A);
								}
								if (!empty($record_details)) {
									$return_data = array(
										'status' => 'FATAL',
										'message' => !empty($form_object->form_options['antibot-error']) ? esc_html($form_object->form_options['antibot-error']) : esc_html__('Thank you. We have already got your request.', 'lepopup')
									);
								} else {
									$payment_ok = false;
									$log_record = $form_object->save_data($hash);
