<?php
/* Klaviyo integration for Green Forms */
...
class lepopup_klaviyo_class {
	...
	var $location_parameters = array('city', 'region', 'country', 'zip');
	var $fields_meta;
	...
	function admin_settings_html() {
		...
		$html .= '				
			...
			<div class="lepopup-properties-item">
				<div class="lepopup-properties-label">
					<label>'.esc_html__('Submit as subscribed', 'lepopup').'</label>
				</div>
				<div class="lepopup-properties-tooltip">
					<i class="fas fa-question-circle lepopup-tooltip-anchor"></i>
					<div class="lepopup-tooltip-content">'.esc_html__('Control whether a profile is submitted as subscribed.', 'lepopup').'</div>
				</div>
				<div class="lepopup-properties-content">
					<input class="lepopup-checkbox-toggle" type="checkbox" value="on" id="subscribe-'.esc_html($checkbox_id).'" name="subscribe"'.($data['subscribe'] == 'on' ? ' checked="checked"' : '').' /><label for="subscribe-'.esc_html($checkbox_id).'"></label>
				</div>
			</div>';
			$return_object = array();
	...
	function admin_lists() {
		...
		if (current_user_can('manage_options')) {
			...
			$result = $this->connect($deps['api-key'], 'api/lists');
			if (is_array($result)) {
				if (array_key_exists("errors", $result) && !empty($result['errors'])) {
					$return_object = array('status' => 'ERROR', 'message' => esc_html($result['errors'][0]['detail']));
					echo json_encode($return_object);
					exit;
				}
				...
				foreach($result['data'] as $list) {
					if (is_array($list)) {
						if (array_key_exists('id', $list) && array_key_exists('attributes', $list) && array_key_exists('name', $list['attributes'])) {
	...
	function front_submit($_result, $_data) {
		...
		foreach ($data['fields'] as $key => $value) {
			if (!empty($value) && $key != 'email') {
				if (in_array($key, $this->location_parameters)) {
					$post_data['attributes']['location'][$key] = $value;
				} else {
		...
		if (!empty($properties)) {
			$post_data['attributes']['properties'] = $properties;
		}

		$result = $this->connect($data['api-key'], 'api/profiles?filter=equals(email,'.rawurlencode("'".$data['fields']['email']."'").')');
		if (empty($result) || !array_key_exists("data", $result) || empty($result['data'])) {
			$result = $this->connect($data['api-key'], 'api/profiles', array('data' => $post_data), 'POST');
			$contact_id = $result['data']['id'];
		} else {
			$contact_id = $result['data'][0]['id'];
			$post_data['id'] = $contact_id;
			$result = $this->connect($data['api-key'], 'api/profiles/'.$contact_id, array('data' => $post_data), 'PATCH');
		}
		if ($data['subscribe'] == 'on') {
			...
			$result = $this->connect($data['api-key'], 'api/profile-subscription-bulk-create-jobs', array('data' => $post_data), 'POST');
		}
		return $_result;
	}
	...
	function connect($_api_key, $_path, $_data = array(), $_method = '') {
		...
		try {
			...
			$headers = array(
				'Authorization: Klaviyo-API-Key '.$_api_key,
			...
			$lepopup->add_integrations_log_record(
				array(
					'provider' => 'klaviyo',
					'url' => $url,
					'method' => $method,
					...
					'request_content' => json_encode($_data),
