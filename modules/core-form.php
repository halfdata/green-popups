...
class lepopup_form {
	...
	public function save_data($_hash = '') {
		global $lepopup, $wpdb;
		...
		$wpdb->query("INSERT INTO ".$wpdb->prefix."lepopup_records (form_id, personal_data_keys, unique_keys, fields, info, extra, status, str_id, amount, currency, hash, created, deleted) VALUES (
			'".esc_sql($this->id)."','','".esc_sql($unique_keys)."','".esc_sql(json_encode($fields))."','".esc_sql(json_encode($this->form_info))."','".esc_sql(json_encode($this->form_extra))."','".$status."','".esc_sql($str_id)."','0','USD','".esc_sql($_hash)."','".esc_sql(time())."','0')");
			...
			foreach ($notifications as $notification) {
				if ($notification['enabled'] == 'on' && $notification['action'] == $_action) {
					...
					if (!empty($to)) {
						...
						$mail_headers .= "Reply-To: ".esc_html($reply_email)."\r\n";
	...
	<div class="lepopup-uploader-engine">
		...
		<iframe data-loading="false" id="lepopup-iframe-%%upload-id%%" name="lepopup-iframe-%%upload-id%%" src="about:blank" onload="lepopup_uploader_finish(this);" style="display: none !important; width: 0 !important; height: 0 !important;"></iframe>
					...
					case "select":
						...
						$html .= "<div class='lepopup-element lepopup-element-".esc_html($this->form_elements[$i]['id'])."' data-type='".esc_html($this->form_elements[$i]["type"])."' data-deps='".(array_key_exists($this->form_elements[$i]['id'], $this->form_dependencies) ? esc_html(implode(',', $this->form_dependencies[$this->form_elements[$i]['id']])) : '')."'".($this->form_elements[$i]['dynamic-default'] == 'on' ? " data-dynamic='".esc_html($this->form_elements[$i]['dynamic-parameter'])."'" : "")." data-id='".esc_html($this->form_elements[$i]['id'])."' data-top='".intval($this->form_elements[$i]['position-top'])."' data-left='".intval($this->form_elements[$i]['position-left'])."' data-animation-in='".esc_html($this->form_elements[$i]["animation-in"])."' data-animation-out='".esc_html($this->form_elements[$i]["animation-out"])."' style='animation-duration:".intval($this->form_elements[$i]['animation-duration'])."ms;animation-delay:".intval($this->form_elements[$i]['animation-delay'])."ms;z-index:".intval($zindex_base+$seqs[$k]).";top:".intval($this->form_elements[$i]['position-top'])."px;left:".intval($this->form_elements[$i]['position-left'])."px;width:".(intval($this->form_elements[$i]['size-width']) > 0 ? intval($this->form_elements[$i]['size-width'])."px" : "auto").";height:".(intval($this->form_elements[$i]['size-height']) > 0 ? intval($this->form_elements[$i]['size-height'])."px" : "auto").";'><div class='lepopup-input".esc_html($extra_class)."'><select name='lepopup-".esc_html($this->form_elements[$i]['id'])."' class='".esc_html($this->form_elements[$i]["css-class"])."' data-default='".esc_html($default)."' autocomplete='".esc_html($this->form_elements[$i]["autocomplete"])."' aria-label='".esc_html($this->form_elements[$i]["name"])."' onchange='lepopup_input_changed(this);".($this->form_elements[$i]['submit-on-select'] == "on" ? "lepopup_submit(this);" : "")."' onclick='lepopup_input_error_hide(this);'>".$options."</select></div></div>";
						break;
					...
					case "imageselect":
						...
						for ($j=0; $j<sizeof($this->form_elements[$i]["options"]); $j++) {
							...
							$options .= "<input class='lepopup-imageselect' type='".esc_html($this->form_elements[$i]['mode'])."' name='lepopup-".esc_html($this->form_elements[$i]['id']).($this->form_elements[$i]['mode'] == 'checkbox' ? "[]" : "")."' id='".esc_html("lepopup-imageselect-".$id."-".$i."-".$j)."' value='".esc_html($this->form_elements[$i]["options"][$j]["value"])."'".$selected." data-default='".(empty($selected) ? 'off' : 'on')."' onchange='lepopup_input_changed(this);".($this->form_elements[$i]['mode'] == 'radio' && $this->form_elements[$i]['submit-on-select'] == "on" ? "lepopup_submit(this);" : "")."' /><label for='".esc_html("lepopup-imageselect-".$id."-".$i."-".$j)."' onclick='lepopup_input_error_hide(this);".($this->form_elements[$i]['mode'] == 'radio' && $this->form_elements[$i]['submit-on-select'] == "on" ? "if(jQuery(this).prev().is(\":checked\")){lepopup_submit(this);}" : "")."'><span class='lepopup-imageselect-image' style='background-image: url(".esc_html($this->form_elements[$i]["options"][$j]["image"]).");'></span>".$properties['image-label']."</label>";
						}
					...
					case "radio":
						...
						for ($j=0; $j<sizeof($this->form_elements[$i]["options"]); $j++) {
							$selected = "";
							if (array_key_exists("default", $this->form_elements[$i]["options"][$j]) && $this->form_elements[$i]["options"][$j]["default"] == "on") $selected = " checked='checked'";
							$option = "<div class='lepopup-cr-box'><input class='lepopup-radio lepopup-radio-".esc_html($this->form_options["radio-view"])." lepopup-radio-".esc_html($properties["radio-size"])."' type='radio' name='lepopup-".esc_html($this->form_elements[$i]['id'])."' id='".esc_html("lepopup-radio-".$id."-".$i."-".$j)."' value='".esc_html($this->form_elements[$i]["options"][$j]["value"])."'".$selected." data-default='".(empty($selected) ? 'off' : 'on')."' onchange='lepopup_input_changed(this);".($this->form_elements[$i]['submit-on-select'] == "on" ? "lepopup_submit(this);" : "")."' /><label for='".esc_html("lepopup-radio-".$id."-".$i."-".$j)."' onclick='lepopup_input_error_hide(this);".($this->form_elements[$i]['submit-on-select'] == "on" ? "if(jQuery(this).prev().is(\":checked\")){lepopup_submit(this);}" : "")."'></label></div>";
					...
					case "tile":
						...
						for ($j=0; $j<sizeof($this->form_elements[$i]["options"]); $j++) {
							$selected = "";
							if (array_key_exists("default", $this->form_elements[$i]["options"][$j]) && $this->form_elements[$i]["options"][$j]["default"] == "on") $selected = " checked='checked'";
							$option = "<div class='lepopup-tile-box'><input class='lepopup-tile lepopup-tile-".esc_html($properties["size"])."' type='".$this->form_elements[$i]['mode']."' name='lepopup-".esc_html($this->form_elements[$i]['id']).($this->form_elements[$i]['mode'] == 'checkbox' ? "[]" : "")."' id='".esc_html("lepopup-tile-".$id."-".$i."-".$j)."' value='".esc_html($this->form_elements[$i]["options"][$j]["value"])."'".$selected." data-default='".(empty($selected) ? 'off' : 'on')."' onchange='lepopup_input_changed(this);".($this->form_elements[$i]['mode'] == 'radio' && $this->form_elements[$i]['submit-on-select'] == "on" ? "lepopup_submit(this);" : "")."' /><label for='".esc_html("lepopup-tile-".$id."-".$i."-".$j)."' onclick='lepopup_input_error_hide(this);".($this->form_elements[$i]['mode'] == 'radio' && $this->form_elements[$i]['submit-on-select'] == "on" ? "if(jQuery(this).prev().is(\":checked\")){lepopup_submit(this);}" : "")."'>".esc_html($this->form_elements[$i]["options"][$j]["label"])."</label></div>";
					...
					case "star-rating":
						...
						for ($j=$this->form_elements[$i]['total-stars']; $j>0; $j--) {
							$options .= "<input type='radio' name='lepopup-".esc_html($this->form_elements[$i]['id'])."' id='".esc_html("lepopup-radio-".$id."-".$i."-".$j)."' value='".esc_html($j)."'".($this->form_elements[$i]['default'] == $j ? " checked='checked'" : "")." data-default='".($this->form_elements[$i]['default'] == $j ? 'on' : 'off')."' onchange='lepopup_input_changed(this);".($this->form_elements[$i]['submit-on-select'] == "on" ? "lepopup_submit(this);" : "")."' /><label for='".esc_html("lepopup-radio-".$id."-".$i."-".$j)."' onclick='lepopup_input_error_hide(this);'></label>";
						}
						$extra_class = "";
...
*/		
		...
		if (!empty($webfonts) && $lepopup->options['googlefonts-enable'] == 'on') {
