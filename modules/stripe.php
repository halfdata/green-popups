...
class lepopup_stripe_class {
	...
	function front_submit($_result, $_data) {
		...
		try {
				...
				'line_items' => [
					[
						'price_data' => [
							...
							'unit_amount' => intval($data['amount']*$multiplier),
							'product_data' => [
			...
			$html = '
			<form action="'.$stripe_session->url.'" method="get" target="_top" style="display: none !important;">
				<input type="submit" class="lepopup-pay" value="Submit">
			</form>';		
			return array('status' => 'OK', 'form' => $html, 'amount' => number_format($data['amount'], 2, '.', ''), 'currency' => $data["currency"], 'gateway-id' => $data['id']);
		} catch(Exception $e) {
			$body = $e->getJsonBody();
