...
function lepopup_safe_url(raw) {
	if (typeof raw !== "string") return "";
	var trimmed = raw.replace(/[\x00-\x1F\s]+/g, "");
	try {
	...
	// linklocker - begin
			...
			if (idx > 0) {
				...
				if (encoded_url.length > 0) {
					encoded_url = lepopup_decode64(encoded_url);
					if (item_slug == "") {
						var lepopup_safe = lepopup_safe_url(encoded_url);
						if (lepopup_safe) {
							location.href = lepopup_safe;
							redirecting = true;
						}
					} else {
						var lepopup_cookie = lepopup_read_cookie("lepopup-submit-"+item_slug);
				...
				if (data.status == "OK") {
					...
					switch (data.type) {
						case 'redirect':
							...
							var lepopup_safe = lepopup_safe_url(data.url);
							if (lepopup_safe) location.href = lepopup_safe;
							break;
						case 'payment':
							if (data['reset-form'] == "on") lepopup_reset_form(form_uid);
